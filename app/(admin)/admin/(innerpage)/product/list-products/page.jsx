...
const ListProductsPage = () => {
  ...
  useEffect(() => {
    fetchProducts();
  }, [searchQuery, currentPage, sortBy, categoryFilter, subcategoryFilter, statusFilter]);
    ...
    <div className="w-full p-4 space-y-6 max-w-7xl mx-auto">
                        ...
                        <div className="flex justify-center items-center space-x-2">
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  onClick={() => router.push(`/admin/products/edit-product/${item.id}`)}
                                  variant="ghost"
                                  size="icon"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
              ...
              <Card key={item.id} className="overflow-hidden hover:shadow-md transition-shadow">
                ...
                <CardHeader className="p-4 pb-2">
                  ...
                  <CardDescription className="flex items-center gap-1 text-sm">
                    <Tag size={14} />
                    {item.SubCategory?.Category?.catName} &gt; {item.SubCategory?.subcategory}
                  </CardDescription>
                </CardHeader>
                ...
                <CardContent className="p-4 pt-0">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <div className="flex items-center">
                      ...
                      <span className="font-medium text-gray-900">{formatPrice(item.price_rupees)}</span>
                    </div>
            ...
            <div className="col-span-full p-12 flex flex-col items-center justify-center bg-white rounded-lg">
              ...
              <p className="text-gray-500 mb-4">Add a new product or try changing your search filters</p>
              <Link href="/admin/product/add-product">
                <Button className="bg-indigo-600 hover:bg-indigo-700">Create New Product</Button>
              </Link>
            </div>
          )}
        </div>
      ...
      <Dialog open={!!previewProduct} onOpenChange={(open) => !open && setPreviewProduct(null)}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="flex flex-wrap gap-2">
              <Badge variant="outline" className="flex items-center">
                ...
                {previewProduct?.SubCategory?.Category?.catName} &gt; {previewProduct?.SubCategory?.subcategory}
              </Badge>
          ...
          {previewProduct?.ProductImages && previewProduct.ProductImages.length > 0 && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
            <div>
              ...
              <div className="mt-1 space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{previewProduct?.base_price} ₹</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Price (₹):</span>
                  <span className="font-medium">{previewProduct?.price_rupees} ₹</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Price ($):</span>
                  <span className="font-medium">${previewProduct?.price_dollars}</span>
                </div>
                {previewProduct?.tax && (
                  <div className="flex justify-between">
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-500">Inventory Information</h3>
              <div className="mt-1 space-y-2">
                <div className="flex justify-between">
                  <span className="text-gray-600">Stock:</span>
                  <span className="font-medium">{previewProduct?.stock_count} units</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{previewProduct?.quantity_limit}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">HSN Code:</span>
                  <span className="font-medium">{previewProduct?.hsn_code || "N/A"}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 flex gap-4">
              <div className="flex items-center">
                ...
                <span>{previewProduct?.free_shipping === "yes" ? "Free Shipping" : "Paid Shipping"}</span>
              </div>
              <div className="flex items-center">
                <ShoppingCart size={16} className="text-gray-400 mr-2" />
                <span>{previewProduct?.cod === "yes" ? "COD Available" : "No COD"}</span>
              </div>
          ...
          {/* Product Attributes */}
          {previewProduct?.ProductAttributes && previewProduct.ProductAttributes.length > 0 && (
            <div className="mt-3">
              <h3 className="text-sm font-medium text-gray-500">Product Attributes</h3>
              <Accordion type="single" collapsible className="mt-1">
                <AccordionItem value="attributes">
                  <AccordionTrigger className="text-sm">View All Attributes</AccordionTrigger>
                  <AccordionContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {previewProduct.ProductAttributes.map((attr) => (
                        <div key={attr.id} className="border p-2 rounded-md">
                          <div className="font-medium">{attr.Attribute.display_name}</div>
                          <div className="text-sm text-gray-600 mt-1">
                            {attr.ProductAttributeValues.map((attrValue) => (
                              <div key={attrValue.id} className="flex justify-between items-center">
                                <span>{attrValue.AttributeValue.display_value}</span>
                                {(attrValue.price_adjustment_rupees || attrValue.price_adjustment_dollars) && (
                                  <div className="text-xs text-gray-500">
                                    {attrValue.price_adjustment_rupees && `+₹${attrValue.price_adjustment_rupees}`}
                                    {attrValue.price_adjustment_dollars && ` / +${attrValue.price_adjustment_dollars}`}
                                  </div>
                                )}
                              </div>
                            ))}
          ...
          {/* Product Variants */}
          {previewProduct?.hasVariants && previewProduct.ProductVariants && previewProduct.ProductVariants.length > 0 && (
            <div className="mt-3">
              <h3 className="text-sm font-medium text-gray-500">Product Variants ({previewProduct.ProductVariants.length})</h3>
              <Accordion type="single" collapsible className="mt-1">
                <AccordionItem value="variants">
                  <AccordionTrigger className="text-sm">View All Variants</AccordionTrigger>
                  <AccordionContent>
                          ...
                          <div className="flex flex-col md:flex-row gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-sm">
                                <div className="flex justify-between">
                                  <span className="text-gray-600">Price (₹):</span>
                                  <span className="font-medium">{variant.price_rupees} ₹</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-gray-600">Price ($):</span>
                                  <span className="font-medium">${variant.price_dollars}</span>
                                </div>
                                {variant.weight && (
                                  <div className="flex justify-between">
                              ...
                              {/* Variant attributes */}
                              {variant.VariantAttributeValues && variant.VariantAttributeValues.length > 0 && (
                                <div className="mt-2 flex flex-wrap gap-1">
                                  {variant.VariantAttributeValues.map((attrValue) => (
                                    <Badge key={attrValue.id} variant="outline" className="bg-gray-50">
                                      {attrValue.AttributeValue.Attribute.display_name}: {attrValue.AttributeValue.display_value}
                                    </Badge>
                                  ))}
                                </div>
                              )}
          ...
          {/* SEO Information */}
          {(previewProduct?.meta_title || previewProduct?.meta_keywords || previewProduct?.meta_description) && (
            <div className="mt-3">
              <h3 className="text-sm font-medium text-gray-500">SEO Information</h3>
              <Accordion type="single" collapsible className="mt-1">
                <AccordionItem value="seo">
                  <AccordionTrigger className="text-sm">View SEO Details</AccordionTrigger>
                  <AccordionContent>
                    <div className="space-y-2 text-sm">
                      {previewProduct?.meta_title && (
                        <div>
                          <span className="font-medium">Meta Title:</span>
                          <p className="mt-1 text-gray-700">{previewProduct.meta_title}</p>
                        </div>
                      )}
                      {previewProduct?.meta_keywords && (
                        <div>
                          <span className="font-medium">Meta Keywords:</span>
                          <p className="mt-1 text-gray-700">{previewProduct.meta_keywords}</p>
                        </div>
                      )}
                      {previewProduct?.meta_description && (
                        <div>
                          <span className="font-medium">Meta Description:</span>
                          <p className="mt-1 text-gray-700">{previewProduct.meta_description}</p>
                        </div>
                      )}
                    </div>
                  </AccordionContent>
          ...
          <DialogFooter className="mt-4">
            ...
            {previewProduct && (
              <Button 
                onClick={() => {
                  router.push(`/admin/products/edit-product/${previewProduct.id}`);
      ...
      <AlertDialog open={!!deleteConfirmId} onOpenChange={(open) => !open && setDeleteConfirmId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the product and all its related data including variants, attributes, and images.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
