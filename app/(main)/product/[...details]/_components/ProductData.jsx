...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        if (productData.hasVariants && productData.ProductVariants && productData.ProductVariants.length > 0) {
          // Find default variant or use first one
          const defaultVariant = productData.ProductVariants.find(v => v.is_default) || 
  ...
  useEffect(() => {
    ...
    if (matchingVariant) {
      ...
      if (matchingVariant.ProductImages && matchingVariant.ProductImages.length > 0) {
        setProductImages([...matchingVariant.ProductImages, ...(product.ProductImages || [])]);
      } else {
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      id: product.id,
      title: product.title,
      price: selectedVariant ? selectedVariant.price_rupees : product.price_rupees,
      priceDollars: selectedVariant ? selectedVariant.price_dollars : product.price_dollars,
      ...
      variant: selectedVariant ? {
        ...
        attributes: selectedVariant.VariantAttributeValues ? 
          selectedVariant.VariantAttributeValues
            .filter(vav => vav.AttributeValue && vav.AttributeValue.Attribute)
            .map(vav => ({
  ...
  const currentPrice = selectedVariant ? selectedVariant.price_rupees : (product?.price_rupees || 0);
  const currentPriceDollars = selectedVariant ? selectedVariant.price_dollars : (product?.price_dollars || 0);
  const currentStock = selectedVariant ? selectedVariant.stock_count : (product?.stock_count || 0);
  const currentStockStatus = selectedVariant ? selectedVariant.stock_status : (product?.stock_status || "no");
          ...
          <Breadcrumb className="text-sm text-gray-500">
            <BreadcrumbList>
              ...
              {product.SubCategory?.Category && (
                <>
                  <BreadcrumbItem>
                    <BreadcrumbLink href={`/category/${product.SubCategory.Category.id}`}>
              ...
              {product.SubCategory && (
                <>
                  <BreadcrumbItem>
                    <BreadcrumbLink href={`/subcategory/${product.SubCategory.id}`}>
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                ...
                <Badge variant="outline" className="flex items-center gap-1">
                  ...
                  <span>SKU: {selectedVariant ? selectedVariant.sku : 'N/A'}</span>
                </Badge>
              </div>
            </div>
            ...
            <div className="flex flex-col">
              <div className="flex items-end gap-2">
                ...
                <div className="flex items-center text-gray-500 mb-1">
                  ...
                  <span className="text-lg">{formatPrice(currentPriceDollars)}</span>
                </div>
              </div>
                ...
                {currentStockStatus === "yes" && currentStock > 0 ? (
                  <>
                    ...
                    <span className="text-green-700 font-medium">
                      In Stock {currentStock > 0 && `(${currentStock} available)`}
                    </span>
                  </>
                ) : (
                  <>
            ...
            {product.hasVariants && product.ProductAttributes && (
              <div className="space-y-4">
                ...
                {product.ProductAttributes
                  .filter(attr => attr.Attribute && attr.Attribute.is_variant)
                  .map(attr => (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {attr.ProductAttributeValues &&
                          attr.ProductAttributeValues.map(attrValue => {
                            ...
                            const isSelected = selectedAttributes[attr.attribute_id] === 
                                              attrValue.attribute_value_id.toString();
                            
                            // For color attributes
                            if (attr.Attribute.type === 'color') {
                              const colorCode = attrValue.AttributeValue.color_code || '#ccc';
                              return (
                                <TooltipProvider key={attrValue.id}>
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <button
                                        type="button"
                                        className={`w-8 h-8 rounded-full transition-all ${isSelected ? 'ring-2 ring-offset-2 ring-indigo-600' : 'ring-1 ring-gray-200'}`}
                                        style={{ backgroundColor: colorCode }}
                                        onClick={() => handleAttributeChange(attr.attribute_id.toString(), attrValue.attribute_value_id.toString())}
                                      />
                                    </TooltipTrigger>
                                    <TooltipContent>{attrValue.AttributeValue.display_value}</TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                              );
                            }
                            ...
                            if (attr.Attribute.type === 'size') {
                              return (
                                <Button
                                  ...
                                  className={`min-w-[3rem] ${isSelected ? 'bg-indigo-600 hover:bg-indigo-700' : ''}`}
                                  onClick={() => handleAttributeChange(attr.attribute_id.toString(), attrValue.attribute_value_id.toString())}
                                >
                          ...
                          {attr.ProductAttributeValues && 
                           attr.ProductAttributeValues.map(attrValue => (
                            attrValue.AttributeValue && (
                              <SelectItem key={attrValue.id} value={attrValue.attribute_value_id.toString()}>
                                {attrValue.AttributeValue.display_value}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center">
                ...
                <Button 
                  ...
                  size="icon"
                  type="button"
                  onClick={() => setQuantity(q => Math.min(product.quantity_limit || 10, q + 1))}
                  disabled={quantity >= (product.quantity_limit || 10) || quantity >= currentStock}
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {product.free_shipping === "yes" ? "Free shipping" : "Standard shipping rates apply"}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-12">
          <Tabs defaultValue="description" className="w-full">
            <TabsList className="w-full justify-start border-b rounded-none h-auto p-0 mb-6">
              ...
              {product.hasVariants && product.ProductVariants && product.ProductVariants.length > 0 && (
                <TabsTrigger 
                  value="variants" 
                  className="rounded-none data-[state=active]:border-b-2 data-[state=active]:border-indigo-600 data-[state=active]:shadow-none py-3">
                  Variants
            ...
            {product.terms_condition && (
              <TabsContent value="terms" className="mt-0">
                <Card>
                  <CardContent className="p-6">
                    <div className="prose max-w-none">
                      <div dangerouslySetInnerHTML={{ __html: product.terms_condition || '' }} />
                    </div>
                  </CardContent>
                </Card>
              </TabsContent>
                        ...
                        <thead>
                          <tr className="border-b">
                            ...
                            {product.ProductAttributes && product.ProductAttributes
                              .filter(attr => attr.Attribute && attr.Attribute.is_variant)
                              .map(attr => (
                                <th key={attr.id} className="text-left p-2 font-medium text-gray-600">
                                  {attr.Attribute.display_name}
                                </th>
                              ))
                            }
                          ...
                          {product.ProductVariants.map(variant => (
                            <tr key={variant.id} className="border-b hover:bg-gray-50">
                              ...
                              {product.ProductAttributes && product.ProductAttributes
                                .filter(attr => attr.Attribute && attr.Attribute.is_variant)
                                .map(attr => {
                                  const attrValue = variant.VariantAttributeValues && 
                                    variant.VariantAttributeValues.find(
                                      vav => vav.AttributeValue && 
                                            vav.AttributeValue.Attribute && 
                                            vav.AttributeValue.Attribute.id === attr.attribute_id
                                    );
                                  
                                  return (
                                    <td key={attr.id} className="p-2">
                                      {attrValue && attrValue.AttributeValue ? attrValue.AttributeValue.display_value : '-'}
                                    </td>
                                  );
                                })
                              }
                              ...
                              <td className="p-2">
                                <div className="font-medium">₹{formatPrice(variant.price_rupees)}</div>
                                <div className="text-xs text-gray-500">${formatPrice(variant.price_dollars)}</div>
                              </td>
                              ...
                              <td className="p-2">
                                <Button 
                                  ...
                                  size="sm"
                                  type="button"
                                  disabled={variant.stock_status === "no" || variant.stock_count <= 0}
                                  onClick={() => {
                                    ...
                                    if (variant.VariantAttributeValues) {
                                      variant.VariantAttributeValues.forEach(vav => {
                                        if (vav.AttributeValue && vav.AttributeValue.Attribute) {
                                          attributeSelections[vav.AttributeValue.Attribute.id] = vav.attribute_value_id.toString();
                                        }
                                      });
                                    }
                                    
                                    // Set these as selected attributes
                                    setSelectedAttributes(attributeSelections);
                                    
                                    // Scroll to the top of the product section
                                    if (typeof window !== 'undefined') {
                                      window.scrollTo({
                                        top: document.getElementById('product-options')?.offsetTop || 0,
                                        behavior: 'smooth'
                                      });
                                    }
                                  }}
        ...
        <div className="mt-12 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center py-8 border-y border-gray-200">
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm text-gray-500">
                {product.free_shipping === "yes" ? "Free shipping option" : "Quick delivery"}
              </p>
            </div>
